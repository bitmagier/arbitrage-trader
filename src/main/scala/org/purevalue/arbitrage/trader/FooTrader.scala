...
class FooTrader(traderConfig: Config, tradeRoom: ActorRef, tc: TradeContext) extends Actor {
  ...
  val OrderBundleMinGainInUsdt: Double = traderConfig.getDouble("order-bundle.min-gain-in-usdt")
  val TradeAmountInUsdt: Double = traderConfig.getDouble("order-bundle.trade-amount-in-usdt")
  ...
  def findBestShot(tradePair: TradePair): Either[OrderRequestBundle, NoResultReason] = {
    val availableExchanges: Iterable[String] = tc.tradePairs
    ...
    val tradeAmountBaseAsset: Double = CryptoValue(USDT, TradeAmountInUsdt)
    ...
    for (tradePair: TradePair <- tc.tickers.values.flatMap(_.keys).toSet) {
      if (tc.tickers.count(_._2.keySet.contains(tradePair)) > 1) {
        ...
        findBestShot(tradePair) match {
          ...
          case Left(shot) if shot.bill.sumUSDTAtCalcTime > result.map(_.bill.sumUSDTAtCalcTime).min =>
